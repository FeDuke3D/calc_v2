...
std::list<Token> Model::Parse(std::string source) {
  ...
  while (std::regex_search(source, pmatch, pattern,
    ...
    if (pmatch.str()[0] >= '0' && pmatch.str()[0] <= '9') {
      ...
      if (pmatch.str() == "-")
        elem.first = Funct::kMinus;
      else if (pmatch.str() == "+")
        elem.first = Funct::kPlus;
      else if (pmatch.str() == "*")
        elem.first = Funct::kMult;
      else if (pmatch.str() == "/")
        elem.first = Funct::kDiv;
      else if (pmatch.str() == "^")
        elem.first = Funct::kPow;
      else if (pmatch.str() == "(")
        elem.first = Funct::kLeftPar;
      else if (pmatch.str() == ")")
        elem.first = Funct::kRightPar;
      else if (pmatch.str() == "mod")
        elem.first = Funct::kMod;
      else if (pmatch.str() == "cos")
        elem.first = Funct::kCos;
      else if (pmatch.str() == "sin")
        elem.first = Funct::kSin;
      else if (pmatch.str() == "tan")
        elem.first = Funct::kTan;
      else if (pmatch.str() == "acos")
        elem.first = Funct::kAcos;
      else if (pmatch.str() == "asin")
        elem.first = Funct::kAsin;
      else if (pmatch.str() == "atan")
        elem.first = Funct::kAtan;
      else if (pmatch.str() == "sqrt")
        elem.first = Funct::kSqrt;
      else if (pmatch.str() == "ln")
        elem.first = Funct::kLn;
      else if (pmatch.str() == "log")
        elem.first = Funct::kLog;
      else if (pmatch.str() == "x")
        elem.first = Funct::kVarX;
    }
    res.push_back(elem);
  ...
  while (iter != src->end()) {
    if (IsMathFunction(iter->first)) {
      ...
      if (iter->first != Funct::kLeftPar) {
        ...
        auto j = iter;
        while (!IsNumber(j->first) && j != src->end()) ++j;
        if (j == src->end()) throw SyntaxError();
        while (IsNumber(j->first)) ++j;
...
int Model::Priority(Funct src) {
  int res(6);
  if (IsMathFunction(src))
    res = 1;
  else if (IsUnary(src))
    res = 2;
  else if (src == Funct::kPow)
    res = 3;
  else if (src > Funct::kPlus && src < Funct::kPow)
    res = 4;
  else if (src == Funct::kMinus || src == Funct::kPlus)
    res = 5;
  return res;
}
  ...
  for (auto i : expression_) {
    if (i.first == Funct::kVarX) {
      ...
      if (i.first == Funct::kUMinus) {
        ...
        double val = tmp.top();
        tmp.pop();
        if (i.first == Funct::kMinus)
          tmp.top() -= val;
        else if (i.first == Funct::kPlus)
          tmp.top() += val;
        else if (i.first == Funct::kMult)
          tmp.top() *= val;
        else if (i.first == Funct::kDiv)
          tmp.top() /= val;
        else if (i.first == Funct::kMod)
          tmp.top() = fmod(tmp.top(), val);
        else if (i.first == Funct::kPow)
          tmp.top() = pow(tmp.top(), val);
      } else {
        throw SyntaxError();
      }
    } else {
